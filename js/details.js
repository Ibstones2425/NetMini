...
(function () {
  ...
  function renderDetails(data) {
    ...
    // ── Anime badge ── Anime is in development. If the title is
    // anime, surface a "in development" pill on the details page so
    // users know not to expect playback. The watch page still
    // intercepts the play button and shows a full modal.
    const animeBadge = document.getElementById("details-anime-badge");
    if (animeBadge) {
      const isAnime = isAnimeInline(data);
      animeBadge.hidden = !isAnime;
      if (isAnime) {
        // Override the badge text (originally "Sub/Dub via AniList")
        animeBadge.innerHTML = `
          <span class="anime-badge-icon">🚧</span>
          <span class="anime-badge-text">
            <strong>Anime</strong> · Our anime app is in development
          </span>`;
      }
    }
